...
export function schedule(input: ScheduleInput): ScheduleOutput {
  ...
  // 3. Kahn のトポロジカルソート（taskNo 自然順で決定的）
  const cmpTaskNo = (a: number, b: number) =>
    (taskById.get(a)?.taskNo ?? "").localeCompare(
