...
// シードデータ（prisma/mock-data.ts: gantt-test / wbsId=8 = mockDataGantt）に依存する。
// この WBS は gantt 表示検証用で、タスク・依存関係・マイルストーンを含む。
// （test-project-1 / wbsId=1 は wbsTask が空のため、タスクバー検証には使えない）
// ローカルで `npm run dev`（実DB）を起動した状態で `npm run test:e2e` を実行する。
const GANTT_PATH = "/projects/gantt-test/gantt";

test.describe("gantt", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(GANTT_PATH);
  });
  ...
  test("ビュー切替: ガント ↔ テーブル", async ({ page }) => {
    ...
    await expect(page.getByTestId("gantt-chart")).toBeHidden();
  ...
  test("編集モードでタスクバーをドラッグすると位置が右へ動く", async ({
    ...
    await page.getByTestId("gantt-edit-toggle").click();
    await expect(page.getByTestId("gantt-edit-save")).toBeVisible();
