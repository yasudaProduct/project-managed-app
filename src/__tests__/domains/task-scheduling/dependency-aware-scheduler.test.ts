...
describe("dependency-aware-scheduler", () => {
  ...
  test("FIXED: 完了タスクは固定され、後続はアンカー日から始まる", () => {
    const tasks: SchedulableTask[] = [
      {
        taskId: 1,
        taskNo: "A-001",
    ...
    // アンカー=1/10
    const out = schedule(baseInput(tasks, deps, d(2024, 1, 10)));

    // 完了は固定（再計算しない）
    expect(fmt(out.scheduled.get(1)?.start)).toBe("2024-01-01");
    expect(fmt(out.scheduled.get(1)?.end)).toBe("2024-01-02");
    // 先行終了は過去 → 後続はアンカー日(1/10)から
    expect(fmt(out.scheduled.get(2)?.start)).toBe("2024-01-10");
  });

  test("PARTIAL: 着手中は開始固定・残工数をアンカー日から消化して終了再計算", () => {
    const tasks: SchedulableTask[] = [
      {
        taskId: 1,
        taskNo: "A-001",
        ...
        fixedStart: d(2024, 1, 5), // 実績開始(過去)
        remainingHours: 8, // 残8h
      },
    ];
    // アンカー=1/10(水)
    const out = schedule(baseInput(tasks, [], d(2024, 1, 10)));

    // 開始は固定、終了は 1/10 から残8h消化 = 1/10
    expect(fmt(out.scheduled.get(1)?.start)).toBe("2024-01-05");
    expect(fmt(out.scheduled.get(1)?.end)).toBe("2024-01-10");
  });

  test("PARTIAL: 残工数0なら終了はアンカー日（最初の稼働日）", () => {
    const tasks: SchedulableTask[] = [
      {
        taskId: 1,
        taskNo: "A-001",
        assigneeId: 1,
        schedulingMode: "PARTIAL",
        fixedStart: d(2024, 1, 5),
        remainingHours: 0,
      },
    ];
    // アンカー=1/6(土) → 最初の稼働日 1/8(月)
    const out = schedule(baseInput(tasks, [], d(2024, 1, 6)));

    expect(fmt(out.scheduled.get(1)?.start)).toBe("2024-01-05");
    expect(fmt(out.scheduled.get(1)?.end)).toBe("2024-01-08");
  });
});
