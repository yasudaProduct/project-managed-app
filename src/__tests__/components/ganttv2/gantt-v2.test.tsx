...
describe('GanttV2Component', () => {
  ...
  describe('フィルタリング機能', () => {
    ...
    test('担当者フィルターが正常に動作する', async () => {
      ...
      // 担当者フィルターを変更
      const assigneeSelects = screen.getAllByDisplayValue('すべて');
      const assigneeSelect = assigneeSelects[1]; // 2番目の「すべて」が担当者フィルター
      fireEvent.change(assigneeSelect, { target: { value: 'テストユーザー1' } });
  ...
  describe('エラーハンドリング', () => {
    ...
    test('onTaskUpdateコールバックが正常に呼び出される', () => {
      ...
      render(<GanttV2Component {...defaultProps} onTaskUpdate={mockCallback} />);
...
describe('GanttV2Component - 日付計算', () => {
  test('日付範囲の計算が正確である', () => {
    const { rerender } = render(<GanttV2Component {...defaultProps} />);
    
    // プロジェクト期間 + バッファが正しく計算される
    const expectedStart = new Date('2024-04-24'); // 5月1日 - 7日
    const expectedEnd = new Date('2024-06-07'); // 5月31日 + 7日
    
    // コンソールログで日付範囲を確認
    expect(console.log).toHaveBeenCalledWith('projectStart', mockProject.startDate.toISOString());
    expect(console.log).toHaveBeenCalledWith('projectEnd', mockProject.endDate.toISOString());
  });
