...
jest.mock("@/app/wbs/[id]/gantt/dependency-actions", () => ({
  createGanttDependency: jest.fn(),
  deleteGanttDependency: jest.fn(),
}));
jest.mock("@/app/wbs/[id]/gantt/export-actions", () => ({
  getGanttTasksTsv: jest.fn(),
}));
jest.mock("@/hooks/use-toast", () => ({ toast: jest.fn() }));

// 子コンポーネントは軽量モック（GanttChart は jsdom 非対応APIを多用するため）。
// GanttClient は barrel ではなく各ファイルから直接 import しているため、
// モックも各モジュールパスに対して行う。
jest.mock("@/components/gantt/gantt-chart", () => ({
  GanttChart: ({
    tasks,
    onEditDependencies,
  }: {
...
jest.mock("@/components/gantt/view-switcher", () => ({
  ViewSwitcher: ({
    onViewChange,
  }: {
...
jest.mock("@/components/gantt/quick-actions", () => ({
  QuickActions: () => <div data-testid="quick-actions" />,
}));
jest.mock("@/components/gantt/task-filter-control", () => ({
  TaskFilterControl: () => <div data-testid="task-filter" />,
}));
jest.mock("@/components/gantt/task-detail-sidebar", () => ({
  TaskDetailSidebar: () => null,
}));

jest.mock("@/components/gantt/task-table", () => ({
  TaskTable: ({ tasks }: { tasks: unknown[] }) => (
    <div data-testid="task-table">table-tasks:{tasks.length}</div>
  ),
}));
jest.mock("@/components/gantt/dependency-edit-modal", () => ({
...
describe("GanttClient（統合スモーク）", () => {
  it("マウント時に wbsId 指定で tasks/phases/assignees を取得する", async () => {
    render(<GanttClient wbsId={42} />);
