...
describe("GanttChart", () => {
  ...
  describe("見通しバーの描画", () => {
    ...
    it("showForecast OFF では見通しバーを描画しない", () => {
      ...
      expect(
        container.querySelectorAll('[data-testid="gantt-forecast-bar"]'),
      ).toHaveLength(0);
    });
  ...
  describe("イナズマ線（進捗線）", () => {
    ...
    it("showProgressLine ON でイナズマ線を描画する", () => {
      ...
      expect(queryByTestId("gantt-progress-line")).toBeInTheDocument();
    });
    ...
    it("showProgressLine OFF ではイナズマ線を描画しない", () => {
      ...
      expect(queryByTestId("gantt-progress-line")).not.toBeInTheDocument();
    });
  });
  ...
  describe("編集モードの切替UI", () => {
    it("非編集時は編集モードボタンを表示し、クリックで onEnterEditMode", () => {
      ...
      fireEvent.click(screen.getByTestId("gantt-edit-toggle"));
    ...
    it("初回データ読込中（isDataLoading）は編集モードボタンが無効", () => {
      ...
      expect(screen.getByTestId("gantt-edit-toggle")).toBeDisabled();
    });
    ...
    it("編集中は保存/キャンセルを表示し、各コールバックが発火", () => {
      ...
      expect(
        screen.queryByTestId("gantt-edit-toggle"),
      ).not.toBeInTheDocument();

      fireEvent.click(screen.getByTestId("gantt-edit-save"));
      expect(onSaveEdit).toHaveBeenCalled();
      fireEvent.click(screen.getByTestId("gantt-edit-cancel"));
    ...
    it("保存中は保存ボタンが「保存中...」表示かつ無効", () => {
      ...
      const save = screen.getByTestId("gantt-edit-save");
  ...
  describe("依存関係のドラッグ作成（編集モード）", () => {
    ...
    it("ドラッグ中はプレビュー線とターゲット強調を表示し、確定後に消える", () => {
      ...
      fireEvent.mouseMove(document, TASK2_LEFT_HALF);
      expect(
        container.querySelector('[data-testid="gantt-connect-line"]'),
      ).toBeInTheDocument();
      expect(
        container.querySelector('[data-testid="gantt-connect-target"]'),
      ).toBeInTheDocument();
  ...
  describe("ホバーでツールチップ", () => {
    ...
    it("ドラッグ後にツールチップが古い位置で再表示されない（編集モード）", () => {
      ...
      fireEvent.mouseEnter(bar);
      expect(screen.getByTestId("gantt-task-tooltip")).toBeInTheDocument();
