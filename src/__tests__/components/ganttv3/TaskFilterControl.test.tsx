...
describe("TaskFilterControl", () => {
  it("条件が無いときはカウントバッジを表示しない", () => {
    setup();
    expect(screen.queryByTestId("ganttv3-filter-count")).not.toBeInTheDocument();
  });
  ...
  it("有効な条件数をバッジ表示する", () => {
    ...
    expect(screen.getByTestId("ganttv3-filter-count")).toHaveTextContent("4");
  });

  it("キーワード入力で onChange が発火する", () => {
    const { onChange } = setup();
    fireEvent.click(screen.getByTestId("ganttv3-filter-trigger"));
    fireEvent.change(screen.getByTestId("ganttv3-filter-keyword"), {
      target: { value: "設計" },
    });
  ...
  it("クリアで空フィルタへ戻す", () => {
    ...
    fireEvent.click(screen.getByTestId("ganttv3-filter-trigger"));
    fireEvent.click(screen.getByTestId("ganttv3-filter-clear"));
