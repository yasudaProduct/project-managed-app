...
export class ExcelWbsRepository implements IExcelWbsRepository {
  ...
  private mapToExcelWbs(record: wbs): ExcelWbs {
    return {
      ...
      // 0 は有効値。falsy判定でnullへ落とさない
      KIJUN_KOSU: record.KIJUN_KOSU != null ? Number(record.KIJUN_KOSU) : null,
      YOTEI_KOSU: record.YOTEI_KOSU != null ? Number(record.YOTEI_KOSU) : null,
      JISSEKI_KOSU: record.JISSEKI_KOSU != null ? Number(record.JISSEKI_KOSU) : null,
      KIJUN_KOSU_BUFFER: record.KIJUN_KOSU_BUFFER != null ? Number(record.KIJUN_KOSU_BUFFER) : null,
