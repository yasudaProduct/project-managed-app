...
import type {
  WorkloadData,
  AssigneeWarningData,
  AssigneeGanttResponse,
} from "@/applications/assignee-gantt/workload-data";

/** 表示期間(YYYY-MM-DDローカル)を受け取り作業負荷を返す取得関数 */
export type WorkloadFetcher = (
  startYmd: string,
  endYmd: string
) => Promise<AssigneeGanttResponse>;

interface WorkloadGanttChartProps {
  /** 未指定時はプレビュー専用(previewWorkloads の表示のみ) */
  ...
  /** カードタイトル(既定: 担当者別ガントチャート) */
  title?: string;
  /** タイトル行の右側に差し込む要素(トグル等) */
...
interface TaskAllocation {
  ...
  periodStart?: string;
  periodEnd?: string;
  projectName?: string; // 他WBS合算時の所属プロジェクト
}
...
/**
 * 担当者別作業負荷ガントチャート(汎用)
 * データ取得は fetcher に委譲し、WBS単位・プロジェクト横断のどちらの画面からも利用できる。
 */
export function WorkloadGanttChart({
  ...
  // 警告（実現不可能タスク）
  const [warnings, setWarnings] = useState<AssigneeWarningData[]>([]);
  ...
  // データ取得（fetcher 未指定時はプレビュー専用のため何もしない）
  const fetchWorkloads = useCallback(async () => {
    ...
    try {
      ...
      const result = await fetcher(toYMD(startDate), toYMD(endDate));

      if (!result.success || !result.data) {
        throw new Error(result.error || "作業負荷の取得に失敗しました");
      }

      setWorkloads(convertWorkloadData(result.data));
                  ...
                  {selectedAllocation.taskAllocations.length > 0 ? (
                    <div className="space-y-2">
                      {selectedAllocation.taskAllocations.map((t, i) => (
                        ...
                        >
                          {t.projectName && (
                            <div className="text-xs text-blue-700 bg-blue-50 rounded px-1.5 py-0.5 inline-block mb-1">
                              {t.projectName}
                            </div>
                          )}
