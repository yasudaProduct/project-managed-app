...
- 責務: プロジェクトのライフサイクル、ステータス管理、設定（進捗測定方式・見通し計算方式・EVM除外設定・品質閾値等）
...
### Quality 集約
- `quality_review_target`, `quality_reviewer`, `quality_size_metric`, `quality_finding`
- 責務: 定量品質管理（レビュー対象・指摘・規模メトリクス・品質指標計算）
- 主要エンティティ:
  - `QualityReviewTarget`（wbsId, taskNo, documentType, reviewType）
  - `QualityFinding`（severity, category, description, foundAt）
  - `QualityReviewer`（reviewerUserId, reviewTaskNo）
  - `QualitySizeMetric`（unit, value, measuredAt）
...
| QualityStatus | quality | `NORMAL` / `WARNING` / `DANGER` |
| QualityDocumentType | quality | `DESIGN` / `CODE` / `TEST` / `OTHER` |
| QualityReviewType | quality | `PEER` / `FORMAL` / `INSPECTION` / `OTHER` |
| QualitySizeUnit | quality | `PAGE` / `LINES_OF_CODE` / `TEST_CASE` |
| QualitySeverity | quality | `MAJOR` / `MINOR` / `INFO` |
| QualityThreshold | quality | `warnThreshold` / `dangerThreshold` / `higherIsBetter` |
...
### Quality
- `QualityReviewTarget`: wbsId, taskNo, name は必須
- `QualityFinding`: targetId は必須
- `QualitySizeMetric`: `MAN_HOUR` は保存不可（都度集計）、`value > 0`
...
| QualityMetricsCalculator | quality | レビュー密度・欠陥密度・重大指摘率・完了率の計算 + ステータス評価 |
...
- 主要リポジトリIF:
  ...
  - `IQualityMetricsRepository`, `IQualityReviewTargetRepository`, `IQualityTaskRepository`
...
- **品質管理（Quality）** はWBSを参照するが、FKを貼らず洗い替えインポートから保護（論理結合）
