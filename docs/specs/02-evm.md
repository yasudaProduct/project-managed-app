...
| 方式 | 定数値 | ETC計算式 | 意味 |
|------|--------|-----------|------|
| CPI法 | `CPI_ONLY`（デフォルト） | `(BAC - EV) / CPI` | 現在のコスト効率が今後も継続すると仮定 |
| CPI×SPI法 | `CPI_SPI` | `(BAC - EV) / (CPI × SPI)` | スケジュール遅延もコストに反映 |
| 計画法 | `PLANNED` | `BAC - EV` | 残りの作業は計画通りのコストで完了すると仮定 |

**ゼロ除算時の挙動:**

- `CPI_ONLY`: CPI = 0 の場合、ETC = `0` を返す（`Infinity` ではない）。
- `CPI_SPI`: CPI × SPI = 0 の場合、ETC = `0` を返す。
...
| PV = 0 | SPI = 0（ゼロ除算を回避） |
| AC = 0 | CPI = 0（ゼロ除算を回避） |
| CPI = 0（CPI_ONLY） | ETC = 0（ゼロ除算を回避）、EAC = AC + 0 = AC |
