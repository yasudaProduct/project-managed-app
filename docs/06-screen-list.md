...
| パス | 画面名 | 概要 |
|------|--------|------|
| `/wbs/[id]/gantt` | ガントチャート | 高機能版ガントチャート（クリティカルパス等） |
| `/wbs/[id]/assignee-gantt` | 担当者別ガントチャート | 担当者の作業負荷視覚化。「他WBSの負荷を考慮」トグルで未開始・進行中プロジェクトの最新WBS分を合算表示 |
| `/assignee-gantt` | 担当者負荷（プロジェクト横断） | 未開始・進行中プロジェクトの最新WBSを横断し、ユーザー単位に合算した日別作業負荷を表示 |
...
| `/schedule/free-time` | 共通空き時間検索 | 指定ユーザー全員が空いている時間帯を検索（09:00〜18:00、土日・祝日・会社休日は除外） |
