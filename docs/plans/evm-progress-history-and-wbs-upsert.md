...
- これにより WorkRecord/TaskStatusLog/TaskDependency/QualityReviewTarget の紐づけが切れない。
...
- 実現性：中。**既存クエリへの `isDeleted=false` 反映漏れが最大の注意点**（後述リスク）。QualityReviewTarget の `isActive` 無効化パターンが先行事例（[quality-review-target-prisma.repository.ts](../../src/infrastructures/quality/quality-review-target-prisma.repository.ts)）。
...
| QualityReviewTarget | FKなし | — | `isActive` 同期は現行どおり |
...
- QualityReviewTarget等の orphan/inactive データの定期クリーンアップ方針確定。
...
- **soft-delete の適用範囲の全数洗い出し**（EVM/サマリ/WBS表示/geppoタスクマッピング/品質連携 等、`isDeleted=false` を入れる箇所）。実装着手前に対象クエリ一覧化が必要。`findActiveByWbsId` / `findIncludingDeletedByWbsId` の使い分けも合わせて確定。
...
| soft-deleteの先行事例（品質ターゲット） | [src/infrastructures/quality/quality-review-target-prisma.repository.ts](../../src/infrastructures/quality/quality-review-target-prisma.repository.ts) |
