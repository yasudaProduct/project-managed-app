...
| 実績工数 | **作業実績（work_records）の登録月** で集計。「実際に作業した月」「実際に作業した人」で計上される（タスクの担当者と異なる人が代行した場合、その代行者の行に計上される）。**WBSタスクに紐付かない実績は集計から除外される**（後述「未紐付け実績の扱い」参照） |
...
**WBSタスクに紐付いていない作業実績（`work_records.taskId` が NULL）は、集計表のすべての表から除外されます。** 月別・担当者別集計表の実績欄にも含まれません。

### 紐付かない原因

geppo インポート時、月報の `PROJECT_ID + WBS_NO` をキーに WBS タスクを検索して `taskId` を設定します。次のような状況で紐付けが失敗し、`taskId = NULL` の状態でレコードだけが作成されます。

- geppo の `WBS_NO` がタイポ・空欄
- 対応する WBS タスクがまだ作成されていない（タスク追加前に月報が来た）
- WBS タスクの `taskNo` が変更された／削除された
- 担当者が誤ったプロジェクト ID で月報を提出した

### 確認方法

ヘッダー左側の **未紐付けバッジ**（オレンジ色のバッジ）に件数が表示されます。0件なら集計と実態が一致している状態です。

### 対処方法

1. geppo 側で誤った `WBS_NO` を正しい値に修正、または対応する WBS タスクを WBS 側に追加する
2. 該当月の geppo を再インポートする

→ 紐付けが成立した時点で、該当実績は次回の集計表表示から自動的に取り込まれます。
...
flowchart LR
    ...
    WR[(作業実績<br/>work_records<br/>taskId IS NOT NULL)] --> MonthAlloc
    Schedule[(ユーザースケジュール<br/>休暇等)] --> MonthAlloc
    MonthAlloc --> MonthAssignee[月別・担当者別集計表]
    MonthAlloc --> MonthPhase[月別・工程別集計表]
    Unlinked[(未紐付け実績<br/>taskId IS NULL)] -.除外.-> MonthAlloc
    Unlinked --> Badge[ヘッダーの未紐付けバッジ]
```
...
- **未紐付け実績（WBSタスクに対応しない work_records）は集計表に現れません**。ヘッダーの未紐付けバッジで件数を確認できます（詳細は「未紐付け実績の扱い」セクション参照）
