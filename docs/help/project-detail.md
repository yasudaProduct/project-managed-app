...
タスクの工数を月にどう割り振るかは、プロジェクト設定の **計算モード** によって変わります。
...
予定工数の **全量を「予定開始日」の月** に計上します。短いタスクや単月で完結するタスクが多いプロジェクト向きです。

### 各列の算出方法

| 列 | 算出方法 |
| --- | --- |
| 基準工数 | タスクの **基準工数** を上記モードで月別に按分した値 |
| 予定工数 | タスクの **予定工数** を上記モードで月別に按分した値 |
...
flowchart LR
    ...
    Tasks --> MonthAlloc{月別按分<br/>営業日案分 / 開始日基準}
...
- 担当者別・月別の集計は **作業実績の登録月** が基準です。月をまたいで遡って実績を登録すると、過去月の値が変動します
