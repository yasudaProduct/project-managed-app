...
| 9 | `src/app/wbs/[id]/gantt/actions.ts` | Application Service 利用と Prisma 直叩き（wbs / projectSettings）の混在 | Prisma 部分をサービス経由に置換 |
...
| 6 | `src/app/wbs/[id]/gantt/actions.ts` | TaskProgressCalculator |
| 7 | `src/app/wbs/[id]/gantt/dependency-actions.ts` | DependencyType |
...
| 5 | 日付フォーマット処理が重複（3 方式混在）— **ganttv2 分は解消済み（ganttv2 削除に伴い撤去）** | `src/utils/date-util.ts`（canonical）のほか、`src/applications/task-scheduling/tsv-converter.ts:39`、`src/domains/calendar/assignee-working-calendar.ts:145`、`src/domains/calendar/company-calendar.ts:60`、`src/app/dashboard/_components/active-projects-list.tsx:27`、`upcoming-deadlines.tsx:16`、`src/app/company-holidays/page.tsx:116`、`src/components/evm/evm-timeseries-table.tsx:92`、`src/app/wbs/[id]/assignee-gantt/_components/gantt-header.tsx:16` | `date-util.ts` に集約。手組み `getFullYear/getMonth` 連結（ローカル TZ 依存で UTC 保存値とズレる）は date-fns へ置換 |
...
- **P2-5** ✅ 一部集約: 表示用の重複を `date-util.formatDate` に集約（`'YYYY/MM/DD(曜)'`・`'YYYY年MM月DD日'` を追加、出力バイト一致を実測）。calendar 2 件（日付突合キー）・`gantt-header`（オブジェクト返却）は非適合のため据え置き。`gantt-task-list`（ganttv2）は削除済み。
...
- **P3-8** ✅ 解消: `IEvmService` / `ITaskDependencyService`（各公開8メソッド）を定義し implements。DI バインドと全 consumer を IF 経由に統一（`SYMBOL.EvmService` → `SYMBOL.IEvmService` に改名）。
...
| ~~`src/app/test/gantt/page.tsx`~~ — **解消済み（ganttv2 削除に伴い撤去）** | 開発用スクラッチページが本番ルートに露出。削除 |
...
- **P4-1** ✅ 解消: `iuser-repositroy.ts`→`iuser-repository.ts`、`milestone.interfase.ts`→`imilestone-repository.ts`、`work-recoed.ts`→`work-record.ts`（import 全更新）。
- **P4-2** ✅ 解消（2026-07）: **50+ ファイルの命名規則一括リネーム**を Batch1〜5 に分割して実施。内訳: hooks を `use-<name>.ts`（+空の useSSEConnection 削除）、Server Action ファイル統一（`action.ts`→`actions.ts` 3件・`app/actions/` を機能セグメントへ移動）、components の PascalCase→kebab（16件）、sync/notification の PascalCase→kebab（16件）、ドット区切り `.service`/`.repository`→ハイフン（全件）、Prisma 接頭辞除去（ファイル＋クラス名 3件）、リポジトリ IF 接頭辞統一（`wbs-query-repository`→`iwbs-query-repository`）。ユニットテスト 1292 件 green・本番ビルド成功・レイヤー境界 lint クリーン。
- **P4-3** ⚠️ 一部: `domains/sync/IWbsSyncService.ts`・`excel-sync/WbsSyncApplicationService.ts`（死んだコード）を削除。excel-sync の live IF 2 件は `wbs-sync/` へ移動。`app/test/gantt/page.tsx` と、他から一切参照されていなかった `GanttVisualTestHelper` は ganttv2 削除に伴い削除済み（2026-07）。**据え置き**: `api/notifications/analytics/route.ts` は service-worker.js が使用中のため削除しない。
- **P4-4** ✅ 解消: `forecast-calculation.service.ts` の docs 参照修正（P1 で対応済み）、`message`→`error` 統一（P2-3 で解消済み）、`domains/work-records`→`domains/work-record` フォルダ単数化、`Milestone` を private constructor + `create`/`createFromDb` へ統一。
...
## 残課題・据え置きの整理（2026-07 時点）

P1〜P4 の主要項目は本ブランチで対応済み（各セクションの「対応状況（2026-07 更新）」を参照）。以下は**残課題・一部対応・据え置き**を「やるべき／任意／やらなくてよい」で整理したもの。判断の前提は「低リスクで効果があるか」「破壊リスク・技術的制約がないか」。

### 🔨 A. やるべき残課題（別途対応を推奨）

| # | 項目 | 内容 | 状況 |
|---|---|---|---|
| A-1 | **P4-2 命名規則の一括リネーム** | PascalCase→kebab、ドット区切り→ハイフン、リポジトリ IF（`i<name>-repository.ts`）、hooks（`use-<name>.ts`）、Server Action ファイル統一。**50+ ファイル**＋全 importer 更新 | ✅ **2026-07 対応完了**（Batch1〜5 に分割実施。tsc/build/テスト検証済み）。→ 現時点で「必ずやるべき」残課題はなし |

### 🔍 B. 任意（部分対応の残り。必要になったら／低優先）

| # | 項目 | 現状 | 判断 |
|---|---|---|---|
| B-1 | **P3-6 `notifications/send/page.tsx` の RHF 化** | fetch は Server Action 化済み。フォームは useState のまま | 10 項目＋JSON＋ライブプレビューの複雑フォームで、RHF 化の効果に対しリスクが見合わない。**任意**（触るなら慎重に） |
| B-2 | **P3-7 全 Application Service のエラー処理全面監査** | 明確な違反（wbs-sync の throw/Result 混在）は是正済み。他は概ね docs/08 §5 準拠と判断 | 全面的な意味論監査は未実施。**任意・低優先**（新規/変更時に §5 を都度確認すれば足りる） |
| B-3 | **P2-3 名前付き Result 型の `ActionResult<T>` 統一** | `NotificationActionResult`／`ProgressHistoryActionResult`／`EvmActionResult`／`login` の戻り値。既に失敗キーは `error` 準拠 | 統一の実利が小さく consumer の churn が発生。**任意・低優先** |

### 🧊 C. やらなくてよい（現状維持が正／技術的制約で変更不可）

| # | 項目 | 理由 |
|---|---|---|
| C-1 | **P2-2 `cron/notifications`** | `success=(errors===0)`＋207 マルチステータスの独自セマンティクスが `api-response` helper（`success:true` 固定）に不適合。helper 化すると監視挙動が変わるため**維持** |
| C-2 | **P2-2 `notifications/mark-read`・`sync`** | `public/service-worker.js` 専用。SW は Server Action を呼べないため **API Route 必須**（維持） |
| C-3 | **P2-2 `notifications/subscribe`** | `push-notification.ts` が「保存は API に統一」する意図的設計。`savePushSubscription` Server Action は存在するがバイパスは意図的。**維持**（再検討は任意） |
| C-4 | **P2-2 `notifications/analytics`** | service-worker.js が使用中。console.log スタブだが削除不可。DB 永続化は**機能追加**として別扱い（リファクタ対象外） |
| C-5 | **P2-5 date 整形の残 2 箇所** | `assignee-working-calendar.ts`・`company-calendar.ts`（`YYYY-MM-DD` を突合キーに使用＝区切り変更で破壊）、`gantt-header.tsx`（オブジェクト返却で `formatDate` 不適）。いずれも date-util 非適合のため**維持** |
| C-6 | **P3-9 `console.error` / `console.warn`** | 境界のエラーログとして docs/08 §6 準拠。デバッグ `console.log` は削除済み。**追加整理は不要** |

### メモ
- **A-1（P4-2）は 2026-07 に対応完了**。これによりリファクタとして「必ずやるべき」残課題はなくなった。残るのは B（任意・低優先）と C（現状維持が正）のみ。
- B・C の項目は将来のコードレビュー観点として本ドキュメントに残す。新規コードでは各 docs（03/04/08）の規約に沿うこと。
