...
- **P4-3** ⚠️ 一部: `domains/sync/IWbsSyncService.ts`・`excel-sync/WbsSyncApplicationService.ts`（死んだコード）を削除。excel-sync の live IF 2 件は `wbs-sync/` へ移動。**据え置き**: `app/test/gantt/page.tsx` は visual 回帰テストのフィクスチャ（`GanttVisualTestHelper` が使用）、`api/notifications/analytics/route.ts` は service-worker.js が使用中のため削除しない。
...
| B-4 | **P4-3 `app/test/gantt/page.tsx` の本番ルート露出** | visual 回帰テストのフィクスチャ（`GanttVisualTestHelper` が `/test/gantt` を使用） | 削除は visual テスト廃止とセットのため不可。ただし**本番ルートに露出している点が気になる場合のみ**、dev 限定ガード（`notFound()` 等）や e2e 専用ルートへの移設を検討。**任意** |
...
| C-5 | **P2-5 date 整形の残 3 箇所** | `assignee-working-calendar.ts`・`company-calendar.ts`（`YYYY-MM-DD` を突合キーに使用＝区切り変更で破壊）、`gantt-task-list.tsx`（未設定/文字列パススルーの特殊セマンティクス）、`gantt-header.tsx`（オブジェクト返却で `formatDate` 不適）。いずれも date-util 非適合のため**維持** |
