...
- **P4-2** ⏸️ 見送り（残タスク）: PascalCase→kebab、ドット区切り→ハイフン、リポジトリ IF 統一、hooks の `use-<name>.ts`、Server Action ファイル統一等の**50+ ファイル一括リネーム**。diff が巨大でレビュー性を損なうため、別の集中 PR として分離する方針。
...
| # | 項目 | 内容 | 推奨アプローチ |
|---|---|---|---|
| A-1 | **P4-2 命名規則の一括リネーム** | PascalCase→kebab、ドット区切り→ハイフン、リポジトリ IF（`i<name>-repository.ts`）、hooks（`use-<name>.ts`）、Server Action ファイル統一。**50+ ファイル**＋全 importer 更新 | 低リスクだが diff 巨大でレビュー性を損なうため、**領域ごとに分割した専用 PR**（例: hooks だけ / `domains/sync` だけ / `applications/notification` だけ）で段階実施。tsc/build/テストで都度検証 |
...
### メモ
- 本整理時点でリファクタとして「必ずやるべき」ものは **A-1（P4-2 の命名リネーム）のみ**。それ以外は任意または現状維持が妥当。
