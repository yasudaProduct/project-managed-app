...
### P1-7. UI 層が Domain 層を直接 import（11 ファイル・PR-7 で ESLint 導入時に発覚）

P1-1〜6 の解消後、`no-restricted-imports` を実導入した際に新たに検出された、当初の棚卸し対象外だった UI→Domain 直接依存。ビルドを止めないため、各箇所は `// eslint-disable-next-line no-restricted-imports` ＋ 本項目を指す TODO コメントで一時的に抑制済み。個別に Application Service 経由 or `src/types/` への型移設で解消すること。

| # | ファイル | 内容 |
|---|---|---|
| 1 | `src/app/actions/evm/evm-actions.ts` | `EvmMetrics` / `EvmCalculationMode`（`@/domains/evm/evm-metrics`）、`TaskEvmData`（`@/domains/evm/task-evm-data`）を直接 import |
| 2 | `src/app/api/import-jobs/[id]/execute/route.ts` | `ImportJob`（`@/domains/import-job/import-job`）、`NotificationPriority` / `NotificationChannel`（`@/domains/notification/*`）を直接 import |
| 3 | `src/app/api/import-jobs/route.ts` | `ImportJobType`（`@/domains/import-job/import-job-enums`）を型 import |
| 4 | `src/app/wbs/[id]/actions/forecast-actions.ts` | `ForecastCalculationService` 等（`@/domains/forecast/forecast-calculation.service`）を直接 import・呼び出し |
| 5 | `src/app/wbs/[id]/actions/wbs-task-actions.ts` | `TaskStatus`（`@/domains/task/value-object/project-status`）、`ITaskFactory`（`@/domains/task/interfaces/task-factory`）を直接 import |
| 6 | `src/app/wbs/[id]/ganttv3/action.ts` | `TaskProgressCalculator`（`@/domains/task/task-progress-calculator`）を直接呼び出し |
| 7 | `src/app/wbs/[id]/ganttv3/dependency-actions.ts` | `DependencyType`（`@/domains/task-dependency/task-dependency`）を型 import |
| 8 | `src/app/work-records/geppo/actions.ts` | `GeppoSearchFilters` / `GeppoPaginationOptions`（`@/domains/geppo/types`）を型 import |
| 9 | `src/app/work-records/geppo/page.tsx` | `Geppo`（`@/domains/geppo/types`）を型 import |
| 10 | `src/components/task-scheduling/scheduling-workbench.tsx` | `BaselineMode`（`@/domains/task-scheduling/scheduling-options`）を型 import |
| 11 | `src/hooks/use-forecast-calculation.ts` | `ForecastCalculationOptions` / `ForecastCalculationResult`（`@/domains/forecast/forecast-calculation.service`）を型 import |

対応方針の目安：型のみ利用（# 3, 7, 8, 9, 10, 11）は該当する型を `src/types/` へ移設して置換。クラス/サービスの直接呼び出し（# 1, 2, 4, 5, 6）は Application Service 側に薄いラッパーメソッド／DTO を新設して置換する。
...
`eslint.config.mjs` に ESLint の `no-restricted-imports` を導入し、レイヤー境界を機械的に強制している（設定は docs/04 の「依存ルールの機械的強制」を参照）。導入時に新規発覚した UI→Domain 直接依存は上記 P1-7 として記録し、`eslint-disable-next-line` で個別に一時抑制済み。新規コードでは抑制を追加せず、Application Service 経由でアクセスすること。
